...
public class DPFEtudiant {
    ...
    public ArrayList<Etudiant> Parsing(String in) {
        try {
            ...
            JSONArray Users = reader.getJSONArray("etudiants");
    ...
    public Etudiant getEtudiantById(Integer id){
        ArrayList<Etudiant> all = this.getAllEtudiants();
        for (int i = 0; i <all.size() ; i++) {
            if(all.get(i).getId_etudiant()==id){
                return all.get(i);
            }
        }
        return null;
    }
   public ArrayList<Etudiant> getEtudiantsByGroupe(Integer idg){
       ArrayList<Etudiant> result= new ArrayList<Etudiant>();
       ArrayList<Etudiant> all = this.getAllEtudiants();
       for (int i = 0; i <all.size() ; i++) {
           if(all.get(i).getId_groupe() ==idg){
